function [parent_folder, seed_folder, request_file] = folder_names(path_base, experiment_name, singledata)
	grandfather_folder = sprintf("%s/examples/%s/gap_%g/fixed-%s/%s/cache-%s/ctlg-%g/c2ctlg-%g/alpha-%g/load-%g",...
		path_base, experiment_name, singledata.solutiongap,singledata.fixed_data.name,...
		singledata.topology.name,...
