function [parent_folder, seed_folder, request_file] = folder_names(path_base, experiment_name, singledata)
	grandfather_folder = sprintf("%s/examples/%s/timelimit-%g/fixed-%s/%s/ctlg-%g/c2ctlg-%g/alpha-%g/load-%g",...
		path_base, experiment_name, singledata.timelimit,singledata.fixed_data.name, singledata.topology.name,singledata.catalog_size,
