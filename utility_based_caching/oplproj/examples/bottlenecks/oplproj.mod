...
float M[O][V];
execute{
	for (o in O_LQ) for(a_ in V) M[o][a_] = hmin[2] * d[o][a_];
	for (o in O_HQ) for(a_ in V) M[o][a_] = hmin[4] * d[o][a_];
	for (o in O_BF) for(a_ in V) M[o][a_] = bar_bar_r_BF * d[o][a_];
	for (o in O_OF) for(a_ in V) M[o][a_] = bar_bar_r_OF * d[o][a_];
};
