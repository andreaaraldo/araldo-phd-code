...
void cNEDNetworkBuilder::addSubmodulesAndConnections(cModule *modp)
{
    ...
    if (conns)
        for (NEDElement *child=conns->getFirstChild(); child; child=child->getNextSibling())
			...
			{
				std::cout<<"ciao cnednetworkbuilder: child is "<< *child << 
					"This child should be the erroneous NEDElement representing the link" << endl;
				throw 1;
...
void cNEDNetworkBuilder::doConnectGates(cModule *modp, cGate *srcg, cGate *destg, ConnectionElement *conn)
{
	...
	std::cout<<"ciao cnednetworkbuilder.cc: created channel  "<< *channel << endl;
	throw 1;
...
std::string cNEDNetworkBuilder::getChannelTypeName(cModule *parentmodp, cGate *srcgate, ConnectionElement *conn, const char *channelName)
{
    ...
    if (opp_isempty(conn->getLikeType()))
    {
        ...
        {
			std::cout<<"ciao cnednetworkbuilder.cc: !opp_isempty(conn->getType())"<< endl;
			throw 1;
        ...
        {
			std::cout<<"ciao cnednetworkbuilder.cc: opp_isempty(conn->getType()), "<<"conn="<< conn << endl;
			throw 1;

            bool hasDelayChannelParams = false, hasOtherParams = false;
            ParametersElement *channelparams = conn->getFirstParametersChild();
            if (channelparams) {
                for (ParamElement *param=channelparams->getFirstParamChild(); 
					 param; param=param->getNextParamSibling()
				)
    ...
    {
		std::cout<<"ciao cnednetworkbuilder.cc: !opp_isempty(conn->getLikeType())="<< endl;
		throw 1;
...
cChannel *cNEDNetworkBuilder::createChannel(ConnectionElement *conn, cModule *parentmodp, cGate *srcgate)
{
	...
	std::cout<<"ciao cnednetworkbuilder.cc: channelTypeName="<< channelTypeName << endl;
	throw 1;
