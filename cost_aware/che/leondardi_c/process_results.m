...
metrics={"hit_ratio"};
splits = {"0.5_0.25_0.25","0.333_0.333_0.334","0.25_0.25_0.5"};

alphas = 0.8:0.1:1.2;

expression_for_cost_fraction = "grep -r \"cost_fraction\" %s/results-%s-pi_%g-alpha_%g-split_%s-seed_*.log | cut  -f6 | cut -d'=' -f2";
expression_for_hit_ratio = "grep -r \"phit_of_stage_0\" %s/results-%s-pi_%g-alpha_%g-split_%s-seed_*.log | cut -f3 | cut -f2 -d'='";

for idx_split = 1:length(splits);
	split = splits{idx_split};
	for alpha = alphas
		for j = 1:length(metrics)
			...
			if strcmp(metric,"saving")
				...
				for priceratio = priceratios
					...
					command = sprintf(expression,che_output_folder, "CoA", priceratio,alpha,split);
					[status, output] = system(command,1);
					cost_fraction_CoA = str2num(output);

					command = sprintf(expression,che_output_folder, "pLRU", priceratio,alpha,split);
					[status, output] = system(command,1);
				...
				for idx_decision = 1:length(decisions)
					...
					resume_local = [];
					for priceratio = priceratios
						command = sprintf(expression,che_output_folder, decision, priceratio,alpha,split);
						[status, output] = system(command,1);
			...
			filename = sprintf("%s/%s_che-alpha_%g-split_%s.dat", outfolder, metric,alpha,split);
			header="#";
