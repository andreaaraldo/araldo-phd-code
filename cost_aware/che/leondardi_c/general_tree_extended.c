...
//<aa> Measures the costfraction of stage 0
#ifdef COSTAWARE_MEASURES
double costfraction(long CATALOG, double alpha)
{
	long k;
	double phitc; 
	int stage = 0;
     
	double overall_object_cost_with_cache = 0;
	double overall_object_cost_without_cache = 0;
	for (k=1; k<=CATALOG; k++)
	{
		phitc=phitcond(k, stage );
		...
		double popularity = zipfs_pop(k,alpha);
		double average_object_cost_with_cache = popularity * (1-phitc) * obj_price[k];
		double object_cost_without_cache = popularity * obj_price[k];
		overall_object_cost_with_cache += average_object_cost_with_cache;
		overall_object_cost_without_cache += object_cost_without_cache;
	}
	double cost_fraction = overall_object_cost_with_cache / overall_object_cost_without_cache;
