...
double  comp_pin(long k, long stage ) 
{
  ...
  {
		printf("ERROR: the probability that object %ld is in cache is nan\n", k);
		printf("a=%g, q=%g\n",a, q);
		exit(-1);
  }	
  #endif
...
double  phitcond(long k, int stage ) 
{
  ...
  if(cache_pol==pLRU || cache_pol==CoA)
  {	
		TC2=TC[stage];
		if(stage>0)
		{
...
double hitp(long CATALOG, int stage, double alpha)
{
     ...
     for (k=1; k<=CATALOG; k++){
      ...
      if(DETAILEDPRINT) printf(" stage=%d phit(%ld)=%le price=%g\n",
