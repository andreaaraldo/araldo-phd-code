...
global COEFF_TRIANGULAR; global COEFF_MODERATE;
...
global PROJECTION_NO; global PROJECTION_EUCLIDEAN;
									...
									if !isfield(in,"coefficients_str")
										switch settings.coefficients
											...
											case COEFF_TRIANGULAR
												in.coefficients_str="triang";
											case COEFF_MODERATE
												in.coefficients_str="moderate";
											otherwise
												settings.coefficients
	...
	simname = sprintf("%s/p_%d-ctlg_%.1g-ctlg_eps_%g-alpha0_%g-alpha_eps_%g-lambda_%g-%s-T_%.1g-K_%.1g-%s-norm_%s-coeff_%s-projection_%s-boost_%g-tot_time_%g%s-know_%g-seed_%d",...
	settings.mdat_folder,in.p,in.overall_ctlg,in.ctlg_eps, in.alpha0,   in.alpha_eps,   in.lambda,req_str, in.T,     in.K, settings.method, in.normalize_str, in.coefficients_str, settings.projection_str, settings.boost, in.tot_time,   timeev_str, in.know, settings.seed);
